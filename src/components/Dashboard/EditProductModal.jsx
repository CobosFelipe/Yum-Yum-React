...
const EditProductModal = ({ product, onClose, onSave }) => {
  ...
  const { getFetch, putFetch } = UseCustomFetch();

  // Efecto para cargar las categorías al abrir el modal
  useEffect(() => {
  ...
  // Efecto para setear el producto en el formData
  useEffect(() => {
    setFormData(product);
  }, [product]);
  ...
  const handleSubmit = async (e) => {    
    ...
    try {
      const payload = {
        ...
        fk_category_id: Number(formData.fk_category_id || product.category_id),
      ...
      const response = await putFetch(`${API}/product/edit`, payload, {
        credentials: "include",
      });

      if (response && response.status === "success") {
        toast.success("Producto actualizado con éxito.", { position: "top-center" });
        onSave(response.obj);
      } else {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-white rounded-lg shadow-xl w-full max-w-lg lg:max-w-xl xl:max-w-2xl mx-4 p-6" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex justify-between items-center border-b border-b-violet-600 pb-3">
          <span className="block text-md font-medium text-violet-600">EDITANDO</span>
        ...
        <form onSubmit={handleSubmit} className="mt-4 space-y-4 max-h-[70vh] overflow-y-auto pr-2">
          <div className="flex gap-4">
            ...
            <div className="flex flex-col items-center max-h-[50%] object-contain">
