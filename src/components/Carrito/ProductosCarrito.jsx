  ...
  return (
    <div className="flex flex-col items-center justify-center">
      <div className="w-64 h-64 bg-gray-300 bg-center bg-cover rounded-2xl shadow-md">
        <img src={item.img} alt={item.description} className="w-64 h-64 rounded-2xl" />
      </div>

      <div className={`w-70 -mt-10 overflow-hidden bg-white rounded-2xl shadow-lg border-1 cursor-pointer ${styles.card}`}>
        {/* Nombre del producto */}
        <h3 className="pt-2 font-bold tracking-wide text-center text-gray-800 uppercase">{item.product_name}</h3>
        ...
        <div className="flex items-center justify-center gap-4 pb-2">
          ...
          <h3> * </h3>
          <h3 className="font-bold tracking-wide text-center text-gray-800">$ {(item.cantidad * item.price).toLocaleString("es-CO")}</h3>
        </div>
        ...
        <div className="flex items-center justify-center px-3 py-2 gap-2">
          ...
          <button
            ...
            onClick={() => {deleteToCart(item), showToast("delete-cart", "Producto eliminado del carrito")}}
          >
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="size-5">
              <path
                strokeLinecap="round"
